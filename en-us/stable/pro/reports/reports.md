...
#### NOTE
By default, xlwings Reports overwrites existing values in templates if there is not enough free space for your variable. If you want your rows to dynamically shift according to the height of your array, use [Frames](components_filters.md#frames-multi-column-layout).

#### NOTE
Unlike xlwings, xlwings Reports never writes out the index of pandas DataFrames. If you need the index to appear in Excel, use `df.reset_index()`, see [DataFrames](components_filters.md#dataframes).
...
See also the [`mysheet.render_template (API reference)`](../../api/sheet.md#xlwings.Sheet.render_template) and [`mybook.render_template (API reference)`](../../api/book.md#xlwings.Book.render_template).
