...
Since v0.27.4, xlwings works with locally synced files on OneDrive, OneDrive for Business, and SharePoint. Some constellations will work out-of-the-box, while others require you to edit the configuration via the `xlwings.conf` file (see [User Config](addin.md#user-config-ribbon-config-file)) or the workbook’s `xlwings.conf` sheet (see [Workbook Config](addin.md#workbook-config-xlwings-conf-sheet)).
