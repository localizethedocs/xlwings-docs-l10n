...
To get access to `Run main` (new in v0.16) button or the `RunPython` VBA function, you’ll need the xlwings addin (or VBA module), see [Add-in & Settings](addin.md#add-in-settings).

For new projects, the easiest way to get started is by using the command line client with the quickstart command,
see [Command Line Client (CLI)](command_line.md#command-line-client-cli) for details:
...
Also, `RunPython` does not allow you to return values. To overcome these issues, use UDFs, see [User Defined Functions (UDFs)](udfs.md#user-defined-functions-udfs) - however,
